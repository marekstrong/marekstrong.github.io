...
---
/**
 * TSVer – dataset browser with Plotly charts
 * Requires the page to contain the HTML structure with ids used below (tsver, tsver-card, etc.)
 * Data files:
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildCountryPicker(allNames, onChange, initialSelection) {
    ...
    const select = el('select', { multiple: 'multiple', size: Math.min(4, Math.max(4, allNames.length)) });
    allNames.forEach(n => {
  ...
  async function renderOneSeries(container, seriesName, rangeMeta) {
    ...
    card.appendChild(el('h4', {}, seriesName));
    ...
    try {
      ...
      // Sort country names A→Z (handles accents properly)
      //const collator = new Intl.Collator(undefined, { sensitivity: 'base' });
      //const allNames = cols.map(c => c.name).sort(collator.compare);

      // Default selection stays the same Set of names
      //const defaultNames = chooseDefaultCountries(cols, rows, 8);

      // Build picker with alphabetized list
  ...
  async function renderAllSeries(rec) {
    ...
    for (const key of keys) {
      // pass the array of ranges to highlight (if any)
